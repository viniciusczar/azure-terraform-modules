locals {
  ...
  frontend_ip_configuration_name = "${var.app_gateway_name}-${local.location}-feip"
  gateway_ip_configuration_name  = "${var.app_gateway_name}-${local.location}-gwipc"

  resource_group_name = element(coalescelist(data.azurerm_resource_group.rgrp.*.name, azurerm_resource_group.rg.*.name, [""]), 0)
  location            = element(coalescelist(data.azurerm_resource_group.rgrp.*.location, azurerm_resource_group.rg.*.location, [""]), 0)
}

# Grupo de recursos, VNet, seleção de sub-rede e recursos aleatórios
...
# IP público para gateway de aplicativo
...
# Application Gateway com todos os blocos opcionais
resource "azurerm_application_gateway" "main" {
  name                = lower("${var.app_gateway_name}-${local.location}")
  resource_group_name = local.resource_group_name
  location            = local.location
  enable_http2        = var.enable_http2
  zones               = var.zones
  firewall_policy_id  = var.firewall_policy_id != null ? var.firewall_policy_id : null
  tags                = merge({ "ResourceName" = lower("${var.app_gateway_name}-${local.location}") }, var.tags, )
  ...
  # Configuração do pool de endereços de back-end (obrigatório)
  ...
  # Configurações HTTP de back-end (obrigatório)
  ...
  # Configuração do ouvinte HTTP (obrigatório)
  ...
  # Configuração de regras de roteamento de solicitação (obrigatório)
  ...
  # Configuração do bloco de identidade (opcional)
  # Uma lista com um único ID de identidade gerenciado pelo usuário a ser atribuído
  dynamic "identity" {
  ...
  # Configuração de certificado SSL de autenticação (opcional)
  dynamic "authentication_certificate" {
  ...
  # Configuração de certificado SSL raiz confiável (opcional)
  ...
  # Política SSL para Gateway de Aplicativo (Opcional)
  # O Application Gateway tem três políticas de segurança predefinidas para obter o nível apropriado de segurança
  ...
  # Configuração do certificado SSL (.pfx) (opcional)
  ...
  # Configuração de erro personalizada (opcional)
  dynamic "custom_error_configuration" {
  ...
  # Configuração do conjunto de regras de reescrita (opcional)
  ...
  # Configuração do Firewall do aplicativo Web (WAF) (opcional)
  # Camada para ser “WAF” ou “WAF V2”
...
# diagnóstico de monitoramento do azurerm – PIP e gateway de aplicativo
...
resource "azurerm_storage_management_policy" "example" {
  ...
  rule {
    name    = lower("agw-${var.app_gateway_name}-rule")
    enabled = true
    filters {
      prefix_match = [lower("agw-${var.app_gateway_name}-traffic")]
