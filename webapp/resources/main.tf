locals {
  ...
  ip_address = [for ip_address in var.ips_allowed : {
    name                      = "tf_ip_restriction_cidr_${join("", [1, index(var.ips_allowed, ip_address)])}"
  ...
  subnets = [for subnet in var.subnet_ids_allowed : {
    name                      = "tf_ip_restriction_subnet_${join("", [1, index(var.subnet_ids_allowed, subnet)])}"
    ip_address                = null
  ...
  service_tags = [for service_tag in var.service_tags_allowed : {
    name                      = "tf_service_tag_restriction_${join("", [1, index(var.service_tags_allowed, service_tag)])}"
    ip_address                = null
    virtual_network_subnet_id = null
  ...
  scm_ip_address = [for ip_address in var.scm_ips_allowed : {
    name                      = "tf_scm_ip_restriction_cidr_${join("", [1, index(var.scm_ips_allowed, ip_address)])}"
    ip_address                = ip_address
    virtual_network_subnet_id = null
    service_tag               = null
    subnet_id                 = null
  ...
  scm_subnets = [for subnet in var.scm_subnet_ids_allowed : {
    name                      = "tf_scm_ip_restriction_subnet_${join("", [1, index(var.scm_subnet_ids_allowed, subnet)])}"
    ip_address                = null
    virtual_network_subnet_id = subnet
    service_tag               = null
    subnet_id                 = subnet
  ...
  scm_service_tags = [for service_tag in var.scm_service_tags_allowed : {
    name                      = "tf_scm_service_tag_restriction_${join("", [1, index(var.scm_service_tags_allowed, service_tag)])}"
    ip_address                = null
    virtual_network_subnet_id = null
    service_tag               = service_tag
    subnet_id                 = null
...
#---------------------------------------------------------
# Criação ou seleção de grupo de recursos – O padrão é “true”
#----------------------------------------------------------
...
#---------------------------------------------------------
# Gerando URL SAS de armazenamento - O padrão é "false"
#----------------------------------------------------------
...
resource "azurerm_storage_container" "storcont" {
  count                 = var.enable_backup ? 1 : 0
  name                  = var.storage_container_name == null ? "tf-${var.app_service_name}-backup" : var.storage_container_name
...
#---------------------------------------------------------
# Definição do Plano de Serviço de Aplicativo – O padrão é "true"
#----------------------------------------------------------
resource "azurerm_app_service_plan" "main" {
  name                = var.app_service_plan_name == "" ? format("%s", lower(replace(var.app_service_name, "/[[:^alnum:]]/", ""))) : var.app_service_plan_name
  resource_group_name = local.resource_group_name
...
#---------------------------------------------------------
# Definições de serviço de aplicativo – o padrão é “true”
#----------------------------------------------------------
resource "azurerm_app_service" "main" {
  name                    = lower(format("%s", var.app_service_name))
  resource_group_name     = local.resource_group_name
  location                = local.location
  ...
  tags                    = merge({ "ResourceName" = lower(format("%s", var.app_service_name)) }, var.tags, )
...
#---------------------------------------------------------
# Configuração personalizada de domínio e certificado - O padrão é "true"
#----------------------------------------------------------
...
#---------------------------------------------------------
# Recursos do Application Insights - O padrão é "false"
#----------------------------------------------------------
...
resource "azurerm_application_insights" "main" {
  count               = var.application_insights_enabled && var.application_insights_id == null ? 1 : 0
  name                = lower(format("%s", var.app_insights_name))
  location            = local.location
  resource_group_name = local.resource_group_name
...
#-------------------------------------------------------------
# Associação de Rede Virtual do Serviço de Aplicativo – O padrão é "false"
#-------------------------------------------------------------
